...
SOLVE_TIMING_LOOP = (
    ...
    "try (match goal with [ |- exists _, _ ] => "
    "first [ match goal with [ H : ?i < _ |- _ ] => is_var i; exists (1 + i) end "
    "| match goal with [ H : ?i <= _ |- _ ] => is_var i; exists (1 + i) end "
    "| exists 0 ] end); "
    "handle_ex; "
    "first [ match goal with [ H : ?i <= _ |- exists _, _ ] => exists (1 + i) end "
    "| match goal with [ H : ?i < _ |- exists _, _ ] => is_var i; exists (1 + i) end "
    "| try (exists 0) ]; "
    "try (rewrite msub_nowrap by (psimpl; lia)); try (rewrite N_sub_distr by lia); "
    "repeat split; try assumption; "
    "try (match goal with [ |- exists _, _ ] => eexists; eassumption end); "
    "try (rewrite !N.shiftl_mul_pow2; f_equal; lia); "
    "try (match goal with [ H : ?i <= ?n |- 1 + ?i <= ?n ] => "
    "assert (i = n \\/ i < n) by lia; lia end); "
    "try (psimpl; lia); try lia; try hammer; "
    "try (change 4294967296 with (2 ^ 32); "
    "repeat (match goal with [ B : negb ?c = _ |- context[negb ?c] ] => rewrite B end)); "
    "try (rewrite N_sub_distr; lia); "
    "try (match goal with [ H : ?i <= ?n |- _ ] => replace i with n by lia end; lia); "
    "try (match goal with [ H : ?i < ?n |- _ ] => replace i with (n - 1) by lia end; lia); "
    "try hammer."
)
