...
| 0x1f0 => Some (s R_A2 <= base_addr_b + 4 * len /\
        s R_A3 = base_addr_b + 4 * len /\
        cycle_count_of_trace t' = tslli 2 + tsub + tadd + (s R_A2 - base_addr_b) / 4 * (ttbne + tlw + tlw + taddi + taddi + txor + tand + txor + tsw + tlw + txor + tsw + tjal))
