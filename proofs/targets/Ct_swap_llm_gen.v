...
| 0x1e4 => Some ((s R_A2 = base_addr_b) /\ (s R_A3 = len) /\ (4 * len < 2^32) /\ (cycle_count_of_trace t' = 0))
| 0x1f0 => Some ((s R_A2 = base_addr_b) /\ (s R_A3 = len) /\ (4 * len < 2^32) /\ (s R_A5 <= len) /\ (cycle_count_of_trace t' = tslli 2 + tsub + tadd + (s R_A5) * (ttbne + tlw + tlw + taddi + taddi + txor + tand + txor + tsw + tlw + txor + tsw + tjal) + tfbne))
